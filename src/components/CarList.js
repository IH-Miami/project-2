...
const Carlist = () => {
  ...
  const options = {
    ...
    headers: {
      "X-RapidAPI-Key": process.env.REACT_APP_CAR_API,
  ...
  return (
    <div className="car-list">
      <section className="filter">
        <ul>
          <button onClick={getCars} type="button">
            All
          </button>
        </ul>

        <ul>
          <button onClick={() => click("year")}>Year</button>
        </ul>
        {clickFilter === "year" &&
          uniqueYears.map(function (year) {
            return (
              <li>
                <button onClick={() => filterCars("year", year)}>{year}</button>
              </li>
            );
          })}

        <ul>
          <button onClick={() => click("make")}>Make</button>
        </ul>
        {clickFilter === "make" &&
          uniqueMakes.map(function (make) {
            return (
              <li>
                <button onClick={() => filterCars("make", make)}>{make}</button>
              </li>
            );
          })}

        <ul>
          <button onClick={() => click("model")}>Model</button>
        </ul>
        {clickFilter === "model" &&
          uniqueModels.map(function (model) {
            return (
              <li>
                <button onClick={() => filterCars("model", model)}>
                  {model}
                </button>
              </li>
            );
        ...
        <ul>
          <button onClick={() => click("type")}>Type</button>
        </ul>
        {clickFilter === "type" &&
          uniqueTypes.map(function (type) {
            return (
              <li>
                <button onClick={() => filterCars("type", type)}>{type}</button>
              </li>
            );
          })}
      </section>
